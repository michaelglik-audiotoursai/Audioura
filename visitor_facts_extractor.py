...
def fetch_visitor_info_structured(base_site_url: str, language: str = "en") -> str:
    ...
    # Extract from each page and pick the best result
    _best_facts: Optional[VisitorFacts] = None

    for _text, _lang, _url in _fetched_pages:
        ...
        if _best_facts is None or (
            # Prefer result with more complete data
            (len(facts.hours) > len(_best_facts.hours)) or
            (len(facts.hours) == len(_best_facts.hours) and
             bool(facts.admission) > bool(_best_facts.admission))
        ):
            _best_facts = facts
