@echo off
echo === Complete fix for tour-orchestrator with synchronization ===
...
echo.
echo === Replacing logger references with print statements ===
powershell -Command "(Get-Content c:\temp\new_orchestrator.py) -replace 'logger\.info', 'print' | Set-Content c:\temp\new_orchestrator.py"
powershell -Command "(Get-Content c:\temp\new_orchestrator.py) -replace 'logger\.error', 'print' | Set-Content c:\temp\new_orchestrator.py"
powershell -Command "(Get-Content c:\temp\new_orchestrator.py) -replace 'logger\.warning', 'print' | Set-Content c:\temp\new_orchestrator.py"
powershell -Command "(Get-Content c:\temp\new_orchestrator.py) -replace 'log_api_call\(.*?\)', 'print(f\"API Call to coordinates service\")' | Set-Content c:\temp\new_orchestrator.py"
powershell -Command "(Get-Content c:\temp\new_orchestrator.py) -replace 'log_api_response\(.*?\)', 'print(f\"API Response from coordinates service\")' | Set-Content c:\temp\new_orchestrator.py"
powershell -Command "(Get-Content c:\temp\new_orchestrator.py) -replace 'log_error\(.*?\)', 'print(f\"Error: {e}\")' | Set-Content c:\temp\new_orchestrator.py"

echo.
echo === Copying the fixed file to the local directory ===
copy c:\temp\new_orchestrator.py c:\Users\micha\eclipse-workspace\AudioTours\development\tour_orchestrator_service.py
...
echo.
echo === Done! ===
echo The tour_orchestrator_service.py file has been completely replaced in both the local directory and the container.
echo Your local files are now in sync with the container.
pause