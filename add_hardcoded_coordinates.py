#!/usr/bin/env python3
"""
Script to add hardcoded coordinates for common libraries to the database.
"""
import psycopg2
import sys
...
def add_hardcoded_coordinates():
    """Add hardcoded coordinates for common libraries to the database."""
    try:
        ...
        for tour_id, tour_name, request_string, lat, lng in tours:
            ...
            for name, coords in LIBRARY_COORDINATES.items():
                if (name in tour_name_lower or 
                    name in request_string_lower or 
                    any(word in name for word in tour_name_lower.split()) or
                    any(word in name for word in request_string_lower.split())):
                    coordinates = coords
        ...
        print(f"Error adding hardcoded coordinates: {e}")
        return False

def main():
    """Main function."""
    print("\n===== Adding Hardcoded Coordinates =====\n")
    
    success = add_hardcoded_coordinates()
    
    if success:
        print("\nHardcoded coordinates added successfully!")
    else:
        print("\nFailed to add hardcoded coordinates")
        sys.exit(1)
