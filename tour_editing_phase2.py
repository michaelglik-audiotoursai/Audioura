...
def _lookup_edit_blob(new_tour_id):
    """Return the R2 key for an edited tour UUID, or None."""
    conn = get_db_connection()
    try:
        cur = conn.cursor()
        _ensure_edit_map_table(cur)
        cur.execute("SELECT blob_uri FROM tour_edit_blobs WHERE new_tour_id = %s", (str(new_tour_id),))
        row = cur.fetchone()
        cur.close()
...
def _resolve_tour_from_db(tour_identifier):
    ...
    try:
        edit_key = _lookup_edit_blob(tour_identifier)
    except Exception:
        edit_key = None
    if edit_key:
...
def create_complete_tour_with_preservation(original_tour_path, final_stops_data, tour_id=None, original_stops_dict=None, content_language='en'):
    ...
    # Process all final stops with preservation logic
    for stop_data in final_stops_data:
        ...
        if action == 'preserve' and original_stops_dict:
            # Find original stop number before renumbering
            original_stop_num = None
            for orig_num, orig_data in original_stops_dict.items():
                if orig_data['text_content'] == text_content:
                    original_stop_num = orig_num
                    break
            
            if original_stop_num:
                original_audio = original_tour_path / f"audio_{original_stop_num}.mp3"
            ...
            if audio_parts:
                ...
                if original_stops_dict:
                    for orig_num, orig_data in original_stops_dict.items():
                        if orig_data['text_content'] == text_content:
                            original_audio = original_tour_path / f"audio_{orig_num}.mp3"
                            if original_audio.exists():
                                new_audio = new_tour_path / f"audio_{stop_number}.mp3"
                                shutil.copy2(original_audio, new_audio)
...
def _bulk_save_core(tour_id, data):
    ...
    try:
        ...
        print(f"PRESERVE: Total original stops: {len(original_stops_dict)} - {list(original_stops_dict.keys())}")
        
        # Get original stops set for compatibility
        original_stops = set(original_stops_dict.keys())
        ...
        # CRITICAL FIX: Track deleted stops to prevent re-adding them
        deleted_stops = set()
        for stop_data in stops:
            action = str(stop_data.get('action', '')).lower()
            if action == 'delete':
                deleted_stops.add(stop_data.get('stop_number'))
        
        print(f"DELETE: Deleted stops: {deleted_stops}")
        
        # CRITICAL FIX: Merge original preserved stops with processed stops (excluding deleted ones)
        for stop_num, stop_data in original_stops_dict.items():
            if (not any(s['stop_number'] == stop_num for s in final_stops_data) and 
                stop_num not in deleted_stops):
                # This stop was not in the mobile app request and not deleted - preserve it completely
                final_stops_data.append({
                    'stop_number': stop_num,
                    'text_content': stop_data['text_content'],
                    'is_added': False,
                ...
                print(f"PRESERVE: Added preserved stop {stop_num} to final data")
...
def job_status(tour_id, job_id):
    """Job status poll (app: edit_stop_screen._trackAudioGenerationAndRefresh).

    Editing is synchronous in this service — by the time update-stop returns,
    the work is done. Report 'completed' so the app's polling loop resolves
    immediately. Kept as a real route so the gateway (auth: api_key) has a
    backend to reach instead of 404-ing.
    """
    return jsonify({"status": "completed", "tour_id": tour_id, "job_id": job_id})
...
def download_tour_with_flags(tour_id):
    ...
    if not wants_json and STORAGE_MODE == 'cloud':
        edit_key = _lookup_edit_blob(tour_id)
        if edit_key:
            storage = _get_blob_storage()
            if storage:
                try:
