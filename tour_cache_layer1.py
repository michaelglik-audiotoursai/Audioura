...
======================================================================
Caches tour content by a deterministic key (location + tour_type + stop BUCKET).
L1 = exact match on (location, tour_type, bucket). L2 fuzzy matching is deferred
to New Architecture.
...
def _cache_key(location: str, tour_type: str, total_stops: int) -> str:
    """Generate deterministic cache key: SHA256 of '{location}|{tour_type}|{bucket}'.

    [LOCAL-494] The key is bucketed on stop count, NOT the exact count, so that
    e.g. a 4-stop and a 6-stop request for the same venue resolve to one entry.
    """
    bucket = _stop_bucket(total_stops)
    raw = f"{location.strip().lower()}|{tour_type.strip().lower()}|{bucket}"
...
def get_cached_tour(
    ...
    [LOCAL-494] The cache stores the LARGEST count generated in the bucket. If
    the caller asked for fewer stops than were cached, the tour is trimmed on
    delivery (with seam repair) here — and that trimmed delivery is still a cache
    HIT, not a generation. Returns the (possibly trimmed) tour_content string, or
    None if not found.
    """
