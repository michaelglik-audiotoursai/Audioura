#!/usr/bin/env python3
"""ROUND10: Expand before delete — the missing half of Michael's routine.
...
print("=" * 70)
print("ROUND10: EXPAND BEFORE DELETE (v2 — dedup + stop-count fix)")
print("=" * 70)
...
output_file = os.path.join(PROJECT_ROOT, "tours", "LOCAL253_riviera_2stop_round10.txt")
...
# ======================================================================
print("\n" + "=" * 70)
print("STEP 3: EXPAND BEFORE DELETE (ROUND10 primary — with dedup)")
print("=" * 70)
...
print(f"    generate_tour_text.py is UNMODIFIED by ROUND10 (confirmed: git diff empty).")
...
# ======================================================================
print("\n" + "=" * 70)
print("STEP 7: WRITE RIVIERA_2STOP_ROUND10.md")
print("=" * 70)
...
round5_words = 680
round6_words = 298
round10_words = words_after_expansion

_expansion_working = expanded_count > 0
_stop_names_str = ', '.join(s['title'] for s in stops_r7)

md_content = f"""# French Riviera Cycling Tour - 2 Stops, Round 10 (ROUND10)
...
> **Word counts:** Round 5: {round5_words} | Round 6: {round6_words} | **Round 10: {round10_words}**
...
| **ROUND10** | **{round10_words}** | **{r7_r7}** | **{r7_r8}** | **{r7_r9}** | **{r7_r10}** | **{r7_r1_paras}/{r7_total_paras}** | **${total_cost:.4f}** |
...
round10_path = os.path.join(PROJECT_ROOT, "RIVIERA_2STOP_ROUND10.md")
with open(round10_path, 'w') as f:
    f.write(md_content)

print(f"  Written: {round10_path}")
print(f"  Word count: {round10_words} (R5: {round5_words}, R6: {round6_words})")

# Also save the processed tour text
processed_path = os.path.join(PROJECT_ROOT, "tours", "LOCAL253_riviera_2stop_round10.txt")
with open(processed_path, 'w') as f:
    f.write(processed_tour_text)
print(f"  Written: {processed_path}")

# Save expansion log as JSON evidence
evidence_path = os.path.join(PROJECT_ROOT, "tours", "LOCAL253_riviera_2stop_round10_evidence.json")
...
print(f"  Words: {round10_words} (R5: {round5_words}, R6: {round6_words})")
