@echo off
REM Master script to fix the coordinates issue using OpenAI (direct approach)

echo.
echo ===== Fixing Coordinates Issue with OpenAI (Direct Approach) =====
echo.
echo This script will:
echo 1. Directly add the coordinates endpoint to the tour-generator service
echo 2. Directly modify the tour orchestrator service to use the coordinates endpoint
...
echo.
echo Step 1: Directly adding coordinates endpoint to tour-generator service...
echo.
call add_coordinates_direct.bat

echo.
echo Step 2: Directly modifying tour orchestrator service...
echo.
call modify_tour_orchestrator_direct.bat
