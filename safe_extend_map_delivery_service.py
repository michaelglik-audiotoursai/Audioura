#!/usr/bin/env python3
"""
Script to safely extend the existing map-delivery service with tour delivery endpoints
WITHOUT affecting existing functionality
"""
import re

def main():
    print("Safely extending existing map-delivery service...")
    ...
    try:
        ...
        # Add required imports ONLY if not present (safe additions)
        imports_added = []
        ...
        if "send_file" not in content and "from flask import" in content:
            # Safely update the Flask import to include send_file
        ...
        # Add NEW tour delivery functions at the end (before if __name__ == '__main__')
        tour_delivery_code = '''
...
'''
        ...
        print("\n✅ Successfully extended map_delivery_service.py")
        ...
        print("\n✅ Your existing Flutter app tabs will continue to work exactly as before")
        
    except FileNotFoundError:
        print("❌ Error: map_delivery_service.py not found.")
        print("Please run step1_check_existing_service.bat first to copy the file from the container.")
    except Exception as e:
