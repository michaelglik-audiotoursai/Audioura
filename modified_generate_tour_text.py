        ...
        if info_response.status_code == 200:
            ...
            for i, block in enumerate(poi_blocks):
                ...
                if coordinates_match:
                    ...
                    # If this is the first POI, try to parse the coordinates
                    if i == 0:
                        try:
                            coords_text = coordinates_match.group(1).strip()
                            # Try to extract latitude and longitude with cardinal direction support
                            def parse_coords_from_text(text):
                                # Try comma-separated format with cardinal directions
                                coord_match = re.search(r'(\d+\.\d+)\s*[°]?\s*([NS]).*?(\d+\.\d+)\s*[°]?\s*([EW])', text, re.IGNORECASE)
                                if coord_match:
                                    lat = float(coord_match.group(1))
                                    lat_dir = coord_match.group(2).upper()
                                    lng = float(coord_match.group(3))
                                    lng_dir = coord_match.group(4).upper()
                                ...
                                coords_parts = re.findall(r'-?\d+\.\d+', text)
                                if len(coords_parts) >= 2:
                                    return (float(coords_parts[0]), float(coords_parts[1]))
                                
                                return None
                            
                            parsed_coords = parse_coords_from_text(coords_text)
                            if parsed_coords:
                                first_poi_coordinates = parsed_coords
            ...
            # If we still don't have coordinates for the first POI, try to get them
            if first_poi_coordinates == (None, None):
                print("No coordinates found for first POI, requesting them specifically...")
                
                # Make a specific request for coordinates
                coords_prompt = f"""Please provide the GPS coordinates (latitude and longitude) for {poi_list[0]['name']} at {location}.
...
ONLY provide the coordinates, nothing else."""
                ...
                try:
                    ...
                    if coords_response.status_code == 200:
                        ...
                        # Try to extract latitude and longitude with cardinal direction support
                        def parse_coordinate(coord_text, coord_type):
                            # Try standard format with optional negative sign
                            match = re.search(rf'{coord_type}:\s*(-?\d+\.\d+)', coord_text, re.IGNORECASE)
                            if match:
                                return float(match.group(1))
                            
                            # Try format with cardinal directions
                            match = re.search(rf'{coord_type}:\s*(\d+\.\d+)\s*[°]?\s*([NSEW])', coord_text, re.IGNORECASE)
                            if match:
                            ...
                            if coord_type.lower() == 'latitude' and len(numbers) >= 1:
                                return float(numbers[0])
                            elif coord_type.lower() == 'longitude' and len(numbers) >= 2:
                                return float(numbers[1])
                            
                            return None
                        
                        lat = parse_coordinate(coords_text, 'Latitude')
                        lng = parse_coordinate(coords_text, 'Longitude')
                        
                        if lat is not None and lng is not None:
                            first_poi_coordinates = (lat, lng)
                            print(f"Extracted coordinates for first POI: {first_poi_coordinates}")
                            
                            # Update the POI with the coordinates
                            poi_list[0]["coordinates"] = f"{first_poi_coordinates[0]}, {first_poi_coordinates[1]}"
                    else:
                        print(f"Error getting coordinates: {coords_response.status_code}")
...
Then write the description directly — a flowing, 300-word narrative about the exhibit. Do NOT wrap it in brackets, placeholders, or formatting markers. Just write the prose.
