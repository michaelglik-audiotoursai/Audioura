...
def orchestrate_tour_async(job_id, location, tour_type, total_stops, user_id=None, request_string=None):
    ...
    try:
        ...
        ACTIVE_JOBS[job_id]["progress"] = "Step 1/7: Generating tour text..."
        generate_data = {
            "location": location,
            "tour_type": tour_type,
        ...
        ACTIVE_JOBS[job_id]["progress"] = "Step 2/7: Downloading tour text file..."
        safe_location = location.replace(' ', '_').replace(',', '').replace(':', '').replace('/', '_').replace('\\', '_').lower()
        tour_filename = f"{safe_location}_tour.txt"
        
        download_response = requests.get(f"http://tour-generator:5000/download/{job_id_1}", timeout=30)
        ...
        ACTIVE_JOBS[job_id]["progress"] = "Step 3/7: Uploading to tour processor..."
        with open(tour_file_path, 'rb') as f:
            files = {'file': f}
            upload_response = requests.post("http://tour-processor:5001/upload", files=files, timeout=30)
        
        if upload_response.status_code != 200:
            raise Exception(f"Error uploading file: {upload_response.text}")
        
        # Step 4: Process tour
        ACTIVE_JOBS[job_id]["progress"] = "Step 4/7: Processing tour (text → audio → HTML)..."
        ...
        ACTIVE_JOBS[job_id]["progress"] = "Step 5/7: Downloading Netlify deployment package..."
        ...
        ACTIVE_JOBS[job_id]["progress"] = "Step 6/7: Extracting deployment package..."
        ...
        ACTIVE_JOBS[job_id]["progress"] = "Step 7/7: Storing tour in database..."
...
def track_user_tour(user_id, tour_id, request_string):
    ...
    try:
        print(f"DEBUG: Calling user tracking API for user {user_id}")
        payload = {
            'tour_request': {
                'tour_id': tour_id,
        ...
        print(f"DEBUG: User tracking response: {response.status_code} - {response.text}")
        if response.status_code != 200:
...
if __name__ == '__main__':
    ...
    print(f"Starting Modified Tour Orchestrator Service...")
