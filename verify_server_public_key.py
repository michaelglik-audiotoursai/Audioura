#!/usr/bin/env python3

# RFC 3526 Group 14 parameters
P = int("", 16)
G = 2

def verify_server_keys():
    # Server private key for newsletter 169
    server_private_key = "1855699527122823605301096117990787629509201203106440200973732878362536081808"
    
    # Calculate what the server public key should be
    calculated_public_key = pow(G, int(server_private_key), P)
    calculated_hex = hex(calculated_public_key)[2:]  # Remove 0x prefix
    
    # Server public key from mobile app logs (remove 0x prefix)
    mobile_app_received = ""
    
    print("=== SERVER PUBLIC KEY VERIFICATION ===")
    print(f"Newsletter ID: 169")
    print(f"Server Private Key: {server_private_key}")
    print()
    print(f"Calculated Public Key: {calculated_hex}")
    print(f"Mobile App Received:   {mobile_app_received}")
    print()
    
    if calculated_hex == mobile_app_received:
        print("✅ SERVER PUBLIC KEYS MATCH")
        return True
    else:
        print("❌ SERVER PUBLIC KEYS DO NOT MATCH")
        print("This explains the decryption failure!")
        return False

if __name__ == "__main__":
    verify_server_keys()