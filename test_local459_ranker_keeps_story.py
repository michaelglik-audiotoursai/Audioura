...
# LEAD, 2026-08-13: these were exact-match URLs, and they were INVENTED — the real
# Freud Museum page is /2019/02/04/when-dali-met-freud/, the test asserted
# /2018/07/19/. Against the real 104-result fixture the ranker keeps every source
# these criteria name, and the suite still failed, because it was matching a
# fabricated path string rather than the source. Bound to DOMAIN now: the
# acceptance criterion was always "this source survives", never "this URL string".
MUST_SURVIVE_URLS = [
    'invaluable.com',    # the correct publisher — the fact the tour got wrong
    'freud.org.uk',      # Dalí-Freud meeting, Freud Museum London
    'belvedere.at',      # Dalí-Freud meeting, Belvedere Museum Vienna
]

MUST_NOT_SURVIVE_URLS = [
    'Tamarind',          # workshop founded 1960, unrelated to this stop
    'fridmangallery',    # 2013 NYC gallery, surname collision with donor Boris Fridman
]
...
class TestRankerKeepsTheStory:
    ...
    def test_invaluable_survives(self, all_snippets, stop_record):
        ...
        surviving_urls = ' '.join((s.get('url') or '') + ' ' + (s.get('title') or '') for s in ranked)
...
class TestTierHistogram:
    ...
    def test_tier_histogram(self, all_snippets, stop_record):
        ...
        # LEAD 2026-08-13: the assertions here read the FIXTURE's stored tiers, which
        # are baked in at search time — so they tested the fixture, not the code, and
        # could only pass against a fixture manufactured to contain 'unverified'.
        # R1's actual claim is about the CLASSIFIER: a Wikidata failure must no longer
        # be recorded as untrustworthiness. Assert that, against a forced failure.
        import work_story_searcher as wss
        from work_story_searcher import classify_domain

        _orig = wss._check_wikidata_p856
        try:
            wss._check_wikidata_p856 = lambda d: 'unverified'
            wss._MODULE_DOMAIN_CACHE.pop('freud.org.uk', None)
            tier = classify_domain('freud.org.uk', {})
        finally:
            wss._check_wikidata_p856 = _orig
            wss._MODULE_DOMAIN_CACHE.pop('freud.org.uk', None)

        assert tier == 'unverified', (
            "A Wikidata lookup failure must classify as 'unverified', not 'tier3' — "
            "'we could not check this domain' and 'this domain is untrustworthy' are "
            f"different states. Got {tier!r}."
        )
