...
def execute_sql():
    ...
    try:
        ...
        if sql.strip().upper().startswith('SELECT'):
            ...
            cur.close()
            conn.close()
            logger.info(f"SQL SELECT returned {len(results)} rows")
        ...
        logger.info(f"SQL affected {rows_affected} rows")
        return jsonify({
            'status': 'success',
...
def update_tour():
    ...
    try:
        ...
        cur.execute("SELECT id, secret_id FROM tour_requests WHERE tour_id = %s", (tour_id,))
        tour = cur.fetchone()
        ...
        logger.info(f"Found tour: id={tour[0]}, secret_id={tour[1]}")
        ...
        if rows_affected == 0:
            logger.warning(f"No rows affected when updating tour: {tour_id}")
            return jsonify({'error': f'Failed to update tour with ID {tour_id}'}), 500
        
        logger.info(f"Successfully updated tour: {tour_id}, rows affected: {rows_affected}")
        return jsonify({
            'status': 'success',
            'tour_id': tour_id,
            'rows_affected': rows_affected
