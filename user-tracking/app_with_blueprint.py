...
@api_bp.route('/user/<secret_id>', methods=['PUT'])
...
@api_bp.route('/user/<secret_id>', methods=['DELETE'])
...
@api_bp.route('/user/<secret_id>', methods=['GET'])
...
@api_bp.route('/update_tour', methods=['POST'])
def update_tour():
    """Update tour status directly."""
    try:
        print("update_tour endpoint called")
        data = request.json
        ...
        result = cur.fetchone()
        if result:
            print(f"Updated tour: id={result[0]}, tour_id={result[1]}, status={result[2]}, finished_at={result[3]}")
...
if __name__ == '__main__':
    print("Starting Flask app with blueprint...")
