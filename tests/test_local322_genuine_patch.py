...
This test verifies the patch sentence construction via the extracted helper
_build_material_period_patch (LOCAL-324). It confirms:
...
def _insert_patch_after_first_sentence(description, patch_sentence):
    """Replicate the production insertion logic."""
    first_period_idx = description.find('. ')
    if first_period_idx > 20:
        return (description[:first_period_idx + 2]
                + patch_sentence + " "
                + description[first_period_idx + 2:].lstrip())
    else:
        return patch_sentence + " " + description
...
def simulate_material_patch():
    ...
    if not material_ok and material_english:
        # [LOCAL-324] Use the production helper
        patch_sentence = _build_material_period_patch(material_english, None)

        patched = _insert_patch_after_first_sentence(description, patch_sentence)
    ...
    if not material_ok or not period_ok:
        # [LOCAL-324] Use the production helper
        patch_sentence = _build_material_period_patch(
            material_english if not material_ok else None,
            period_english if not period_ok else None,
        )
