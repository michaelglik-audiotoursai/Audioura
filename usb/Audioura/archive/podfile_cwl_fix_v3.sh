#!/bin/bash
# PRE-ASSIGNMENT 20: PODFILE CWLCATCHEXCEPTION EXCLUSION FIX (v3)
# CRITICAL FIXES: Added flutter pub get after flutter clean + idempotent Podfile editing

# Terminal session recorder
exec > >(tee ~/Desktop/podfile_cwl_fix_session.txt) 2>&1

echo "🍎 iOS AMAZON-Q - PODFILE CWLCATCHEXCEPTION EXCLUSION FIX (v3)"
...
echo ""
echo "=== STEP 2.5: REGENERATE FLUTTER EPHEMERAL FILES (CRITICAL FIX) ==="
flutter pub get || { echo "❌ flutter pub get failed"; exit 1; }
echo "✅ Flutter ephemeral files regenerated (Generated.xcconfig recreated)"
