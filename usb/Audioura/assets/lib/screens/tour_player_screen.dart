...
class _TourPlayerScreenState extends State<TourPlayerScreen> with VoiceMethods {
  ...
  void _openMapForStop(int stopIndex) {
    Navigator.push(
      context,
      MaterialPageRoute(
          ...
          if (snapshot.hasData) {
            return InAppWebView(
                    ...
                    onWebViewCreated: (InAppWebViewController controller) async {
                      ...
                      // A#55: JS→Flutter handler — receives {stop: N} from map button taps in HTML
                      controller.addJavaScriptHandler(
                        handlerName: 'openMap',
                        callback: (args) {
                          final stop = (args.isNotEmpty && args[0] is Map)
                              ? (args[0]['stop'] as num?)?.toInt()
                              : null;
                          if (stop != null && mounted) _openMapForStop(stop);
                        },
                      );
