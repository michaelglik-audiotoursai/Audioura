...
## D339 — The prompt dump answers three rounds of guessing: buried facts, and gpt-3.5-turbo
**2026-08-11, 08:0x.** LOCAL-408's literal prompt dump — the artifact nobody had
looked at — resolves D338's open question and turns up something larger.

**Finding 1: the specifics arrive, buried.** The prompt is **30,479 characters,
353 lines**. The story material sits in the middle of it:
...
So it is possibility 2 of the three LOCAL-408 was asked to distinguish. And the
*form* is worse than the position: the facts are pasted as a numbered dump of raw
SERP results —

```
[9] Joan Miró - Le Lezard aux Plumes d'Or - National Galleries of Scotland
    This is from a series of 15 colour lithographs based on Joan Miró's poem…
```

— so the model is handed ten search results at the halfway mark of a 30K prompt and
expected to notice that one of them contains a story.

**Finding 2, and it may be the whole thing: the prose is generated by
`gpt-3.5-turbo`.** `TOUR_LLM_MODEL` is unset in `.env` and unset in
`docker-compose-master.yml`, and **20 call sites in `generate_tour_text.py` default
to `gpt-3.5-turbo`**. Every stop description, every retry, every repair pass this
week has been written by GPT-3.5.

That reframes eight rounds. Selecting one specific detail from ten search results
buried in a 30K context, and building a narrative around it, is exactly the task
GPT-3.5 is weakest at — and exactly what every round since 397 has been asking it
to do. The gates, the coherence checks and the retrieval are all sound; the writer
is the constraint.

**This is standing check #2 in a new form.** "Grep for a production caller before
believing a module does anything" has a sibling: *check which model is actually
running before concluding the pipeline cannot produce good prose.* Nobody looked
at the model name in eight rounds, including LEAD.

**Not changed unilaterally.** Switching the writer model is a cost and quality
decision with blast radius across every tour type, and Michael is awake — it is his
...
# TRACK B (Windows / GCloud_Storied) — renumbered on merge

Track B was written on the Windows machine while Track A ran here, and both
machines independently allocated **D347**. Track A's D347–D414 keep their numbers
(they are referenced by task files and commit messages); Track B's two entries are
renumbered **D347 → D415** and **D348 → D416**. Nothing else was changed.

**Rule from now on: Track B allocates from D500 up.** Two machines appending to one
file cannot share a counter, and a collision that reaches `DECISIONS.md` is worse
than a gap in the numbering.

## D415 — Track B started; Cloud SQL was 23 tables behind local dev, now at parity
...
## D416 — Track B: mobile selector, orchestrator track-writing, and a live Storied deployment, all same session
