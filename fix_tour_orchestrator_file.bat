@echo off
REM Script to fix the tour orchestrator service file and restart the service

echo.
echo ===== Fixing Tour Orchestrator Service File =====
echo.
echo This script will fix the syntax error in the tour orchestrator service file
echo and restart the service.
echo.
echo Press any key to continue or Ctrl+C to cancel...
pause > nul

python fix_tour_orchestrator_file.py
...
if %ERRORLEVEL% EQU 0 (
  ...
  echo ===== Success! =====
  echo.
  echo The tour orchestrator service file has been fixed and the service has been restarted.
  echo.
  ...
  echo ===== Error =====
  echo.
  echo Failed to fix the tour orchestrator service file.
)